...
# 3. Install required system packages (including libsodium-dev and libxml2-dev)
...
# 4. Configure and install PHP extensions required by Magento 2.4.7
RUN docker-php-ext-configure gd --with-freetype --with-jpeg \
    && docker-php-ext-install -j$(nproc) gd bcmath intl pdo_mysql soap xsl zip sockets sodium opcache

# 5. Enable Apache Mod Rewrite for Magento Routing
RUN a2enmod rewrite

# 6. Install Composer
COPY --from=composer:latest /usr/bin/composer /usr/bin/composer

# 7. Install Xdebug (Disabled by default so it doesn't slow down the site. Use toggle script to enable)
...
# 9. Increase Memory Limit to 10GB
RUN echo "memory_limit = 10G" > /usr/local/etc/php/conf.d/magento-memory.ini

# 10. Update Apache DocumentRoot to point to Magento's /pub folder securely
